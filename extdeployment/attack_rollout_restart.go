...
func StartDeploymentRolloutRestart(body []byte) (*DeploymentRolloutRestartState, *extension_kit.ExtensionError) {
	...
	if cmdErr != nil {
		return nil, extutil.Ptr(extension_kit.ToError(fmt.Sprintf("Failed to execute rollout restart: %s", cmdOut), err))
	}
...
func RolloutRestartStatus(body []byte) (*action_kit_api.StatusResult, *extension_kit.ExtensionError) {
	...
	cmdOut, cmdErr := cmd.CombinedOutput()
	if cmdErr != nil {
		return nil, extutil.Ptr(extension_kit.ToError(fmt.Sprintf("Failed to execute rollout restart status check: %s", cmdOut), err))
	}
