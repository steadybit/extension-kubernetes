	...
	return action_kit_api.ActionDescription{
		...
		TargetSelection: extutil.Ptr(action_kit_api.TargetSelection{
			...
			SelectionTemplates: extutil.Ptr([]action_kit_api.TargetSelectionTemplate{
				{
					Label:       "container",
					Description: extutil.Ptr("Find container by cluster, namespace, deployment, and container name"),
					Query:       "k8s.cluster-name=\"\" AND k8s.namespace=\"\" AND k8s.deployment=\"\" AND k8s.container.name=\"\"",
				},
			}),
		}),
		...
		Parameters: []action_kit_api.ActionParameter{
			{
				Label:        "Duration",
				Description:  extutil.Ptr("The duration of the action. The image will be reverted back to the original value after the action."),
				Name:         "duration",
