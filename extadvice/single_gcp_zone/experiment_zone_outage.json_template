{
  "templateTitle": "GCP Zone outage",
  "templateDescription": "Check what happens when a GCP Availability Zone is down and validate that Kubernetes manages this accordingly by routing the traffic within expected failure rates so that the offered features still work. As soon as the zone is available again, the pod should be ready again within 60s.",
  "placeholders": [
  ...
  "tags": ["Redundancy", "GCP", "Availability Zone", "Advice"],
  "experimentName": "GCP Zone Outage of ${target.attr('gcp.zone', 0)} for ${target.attr('steadybit.label')}",
  "hypothesis": "When GCP Availability Zone ${target.attr('gcp.zone', 0)} is down for ${target.attr('steadybit.label')}, Kubernetes manages this accordingly by routing the traffic within expected failure rates so that the offered features still work. As soon as the zone is available again, the pod is ready within 60s.",
  "lanes": [
    {
      "steps": [
        <#if target.attr('k8s.label.tags.steadybit.com/service-validation')?? && target.attr('k8s.label.tags.steadybit.com/service-validation')=='http'>
      ...
      "steps": [
        {
          ...
          "customLabel": "GIVEN: All pods in ${target.attr('gcp.zone', 0)} are ready",
      ...
      "steps": [
        {
          ...
          "customLabel": "Wait for GCP Zone outage"
        },
        {
          "type": "action",
          "ignoreFailure": false,
          ...
          "customLabel": "WHEN: Zone outage of ${target.attr('gcp.zone', 0)} for ${target.attr('steadybit.label')}",
