{
  ...
  "tags": ["Redundancy", "Azure", "Availability Zone", "Advice"],
  "experimentName": "Azure Zone Outage of ${target.attr('azure.zone', 0)} for ${target.attr('steadybit.label')}",
  "hypothesis": "When Azure Availability Zone ${target.attr('azure.zone', 0)} is down for ${target.attr('steadybit.label')}, Kubernetes manages this accordingly by routing the traffic within expected failure rates so that the offered features still work. As soon as the zone is available again, the pod is ready within 60s.",
  "lanes": [
    {
      "steps": [
        <#if target.attr('k8s.label.tags.steadybit.com/service-validation')?? && target.attr('k8s.label.tags.steadybit.com/service-validation')=='http'>
      ...
      "steps": [
        {
          ...
          "customLabel": "GIVEN: All pods in ${target.attr('azure.zone', 0)} are ready",
    ...
    },
    {
      "steps": [
        ...
        },
        {
          ...
          "customLabel": "WHEN: Zone outage of ${target.attr('azure.zone', 0)} for ${target.attr('steadybit.label')}",
