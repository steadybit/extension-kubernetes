...
const SingleZoneID = "com.steadybit.extension_kubernetes.advice.single-zone"

//go:embed *
var SingleZoneContent embed.FS

func GetAdviceDescriptionSingleZone() advice_kit_api.AdviceDefinition {
	return advice_kit_api.AdviceDefinition{
		Id:                        SingleZoneID,
		Label:                     "Schedule Pods Across Zones",
		...
		Tags:                      &[]string{"kubernetes", "daemonset", "deployment", "statefulset", "cloud", "zone"},
		AssessmentQueryApplicable: "(target.type=\"" + extdaemonset.DaemonSetTargetType + "\" OR target.type=\"" + extdeployment.DeploymentTargetType + "\" OR target.type=\"" + extstatefulset.StatefulSetTargetType + "\") AND \"k8s.label.topology.kubernetes.io/zone\" IS PRESENT",
		Status: advice_kit_api.AdviceDefinitionStatus{
			ActionNeeded: advice_kit_api.AdviceDefinitionStatusActionNeeded{
				AssessmentQuery: "count(\"k8s.label.topology.kubernetes.io/zone\") = 1",
				Description: advice_kit_api.AdviceDefinitionStatusActionNeededDescription{
					Instruction: advice_common.ReadAdviceFile(SingleZoneContent, "instructions.md"),
					Motivation:  advice_common.ReadAdviceFile(SingleZoneContent, "motivation.md"),
					Summary:     advice_common.ReadAdviceFile(SingleZoneContent, "action_needed_summary.md"),
				},
			},
			Implemented: advice_kit_api.AdviceDefinitionStatusImplemented{
				Description: advice_kit_api.AdviceDefinitionStatusImplementedDescription{
					Summary: advice_common.ReadAdviceFile(SingleZoneContent, "implemented.md"),
				},
			},
			ValidationNeeded: advice_kit_api.AdviceDefinitionStatusValidationNeeded{
				Description: advice_kit_api.AdviceDefinitionStatusValidationNeededDescription{
					Summary: advice_common.ReadAdviceFile(SingleZoneContent, "validation_needed.md"),
				},
				Validation: extutil.Ptr([]advice_kit_api.Validation{
					{
						Id:                 "com.steadybit.extension_kubernetes.single-zone.experiment-1",
						Type:               "EXPERIMENT",
						Name:               "Availability Zone Outage",
						ShortDescription:   "When a single zone fails, there are still pods of ${target.attr('steadybit.label')} ready to continue providing offered features.",
						ExperimentTemplate: extutil.Ptr(advice_kit_api.ExperimentTemplate(advice_common.ReadAdviceFile(SingleZoneContent, "experiment_zone_outage.json.ftl"))),
					},
				}),
			},
		},
